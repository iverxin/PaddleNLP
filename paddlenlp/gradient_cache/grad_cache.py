...
class RandContext:
    def __init__(self, *tensors):
        ...
        self.fwd_cpu_state = paddle.rand
        # self.fwd_gpu_devices, self.fwd_gpu_states = get_device_states(*tensors)
        self.fwd_gpu_devices, self.fwd_gpu_states = paddle.get_cuda_rng_state()
...
class GradCache:
    ...
    def split_inputs(self, model_input, chunk_size: int) -> List:
        ...
        if isinstance(model_input, (dict, UserDict)) and all(isinstance(x, Tensor) for x in model_input.values()):
            ...
            return list(model_input.split(chunk_size, axis=0))
    ...
    def model_call(self, model: nn.Module, model_input):
        ...
        :return: model output
        """
        with auto_cast() if self.fp16 else nullcontext():
            if isinstance(model_input, Tensor):
    ...
    def forward_no_grad(
        ...
        model_reps = paddle.concat(model_reps, axis=0)
